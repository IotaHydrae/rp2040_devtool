...
#define MAX_AUDIO_FRAME_SIZE (48000*32/4)
/**********************
 *      TYPEDEFS
 **********************/
struct ffmpeg_context_s {
    ...
    uint32_t audio_src_sample_rate;
    uint32_t audio_dst_sample_rate;

    uint8_t **audio_src_data;
    uint8_t **audio_dst_data;
    uint8_t *audio_dst_buffer;
    uint32_t audio_dst_buffer_size_real;
    enum AVSampleFormat audio_src_format;
    enum AVSampleFormat audio_dst_format;

    int audio_src_ch_layout;
    int audio_dst_ch_layout;
    ...
    pthread_mutex_t audio_dst_mutex;
    pthread_cond_t audio_dst_cond;
};
...
#if EANBLE_THREAD_PLAY
static void *snd_pcm_writei_thread(void *privdata){
    static int count =0;
    struct ffmpeg_context_s *ffmpeg_ctx = (struct ffmpeg_ctx_s *)privdata;

    while(1) {
        pthread_mutex_lock(&ffmpeg_ctx->audio_dst_mutex);
        LV_LOG_WARN("writing a audio frame ... %d", count++);
	    pthread_cond_wait(&ffmpeg_ctx->audio_dst_cond, &ffmpeg_ctx->audio_dst_mutex);
#if USE_ALSA_LIB
        snd_pcm_writei(ffmpeg_ctx->pcm_handle, ffmpeg_ctx->audio_dst_buffer, ffmpeg_ctx->audio_dst_buffer_size_real);
#else
        pcm_writei(ffmpeg_ctx->pcm_hanle, ffmpeg_ctx->audio_dst_buffer, ffmpeg_ctx->audio_dst_buffer_size_real);
#endif
	    pthread_mutex_unlock(&ffmpeg_ctx->audio_dst_mutex);
    }
}
#endif
...
{
    int ret = 0;
    // uint8_t *out[] = {ffmpeg_ctx->audio_dst_buffer};

        /* setting up sound output */
        // period = ffmpeg_get_frame_refr_period_audio(ffmpeg_ctx);
        // LV_LOG_WARN("audio period:%d\n", period);
#if USE_ALSA_LIB
        snd_pcm_hw_params_t *hw_params;

        snd_pcm_hw_params_alloca(&hw_params);
        snd_pcm_hw_params_any(ffmpeg_ctx->pcm_handle, hw_params);
        snd_pcm_hw_params_set_access(ffmpeg_ctx->pcm_handle, hw_params, SND_PCM_ACCESS_RW_INTERLEAVED);
        snd_pcm_hw_params_set_format(ffmpeg_ctx->pcm_handle, hw_params, SND_PCM_FORMAT_S16_LE);
        snd_pcm_hw_params_set_channels(ffmpeg_ctx->pcm_handle, hw_params, ffmpeg_ctx->audio_dec_ctx->channels);
        // int sample_rate = ffmpeg_ctx->audio_dec_ctx->sample_rate;
        LV_LOG_WARN("sample_rate : %d", ffmpeg_ctx->audio_dec_ctx->sample_rate);
        snd_pcm_hw_params_set_rate(ffmpeg_ctx->pcm_handle, hw_params, ffmpeg_ctx->audio_dec_ctx->sample_rate, 0);

        snd_pcm_hw_params(ffmpeg_ctx->pcm_handle, hw_params);

        // int chunk_size;
        // snd_pcm_hw_params_get_period_size(hw_params, &chunk_size, 0);
        // LV_LOG_WARN("chunk_size : %d", chunk_size);

        // snd_pcm_hw_params_set_subformat(ffmpeg_ctx->pcm_handle, hw_params, SND_PCM_SUBFORMAT_STD);
        // snd_pcm_hw_params_set_buffer_size(ffmpeg_ctx->pcm_handle, hw_params, 8000);
        // snd_pcm_hw_params_set_period_size(ffmpeg_ctx->pcm_handle, hw_params, 2000, 0);
        snd_pcm_hw_params_set_period_time(ffmpeg_ctx->pcm_handle, hw_params, 100000, 0);
        snd_pcm_hw_params_set_periods(ffmpeg_ctx->pcm_handle, hw_params, 2, 0);

        // snd_pcm_hw_params_free(hw_params);
        if ( 0 > snd_pcm_prepare(ffmpeg_ctx->pcm_handle)){
            LV_LOG_ERROR("snd pcm prepared error!!!");
        }
#else
        ffmpeg_ctx->pcm_conf.channels = ffmpeg_ctx->audio_dec_ctx->channels;
        ffmpeg_ctx->pcm_conf.rate = ffmpeg_ctx->audio_dec_ctx->sample_rate;
        ffmpeg_ctx->pcm_conf.format = PCM_FORMAT_S16_LE;

        ffmpeg_ctx->pcm_conf.period_size = 80;
        ffmpeg_ctx->pcm_conf.period_count = 4;

        ffmpeg_ctx->pcm_conf.start_threshold = 0;
        ffmpeg_ctx->pcm_conf.stop_threshold =  0;
        ffmpeg_ctx->pcm_conf.silence_threshold = 0;

        ffmpeg_ctx->pcm_handle = pcm_open(0, 0, PCM_OUT, &ffmpeg_ctx->pcm_conf);
        if(ffmpeg_ctx->pcm_handle == NULL) {
            LV_LOG_ERROR("failed to alloc pcm handle!");
        } else if (!pcm_is_ready(ffmpeg_ctx->pcm_handle)) {
            pcm_close(ffmpeg_ctx->pcm_handle);
            LV_LOG_ERROR("failed to open PCM!");
        }
#endif

    // av_opt_set_chlayout(ffmpeg_ctx->audio_swr_ctx, "in_chlayout", &audio_src_ch_layout, 0);
    // av_opt_set_int(ffmpeg_ctx->audio_swr_ctx, "in_sample_rate", ffmpeg_ctx->audio_src_sample_rate, 0);
    // av_opt_set_sample_fmt(ffmpeg_ctx->audio_swr_ctx, "in_sample_fmt", ffmpeg_ctx->audio_src_format, 0);

    // av_opt_set_chlayout(ffmpeg_ctx->audio_swr_ctx, "out_chlayout", &audio_dst_ch_layout, 0);
    // av_opt_set_int(ffmpeg_ctx->audio_swr_ctx, "out_sample_rate", ffmpeg_ctx->audio_dst_sample_rate, 0);
    // av_opt_set_sample_fmt(ffmpeg_ctx->audio_swr_ctx, "out_sample_fmt", ffmpeg_ctx->audio_dst_format, 0);

    // AVFrame * frame = ffmpeg_ctx->frame;
    ret = swr_convert(ffmpeg_ctx->audio_swr_ctx,
                &ffmpeg_ctx->audio_dst_buffer,
                // ffmpeg_ctx->audio_dec_ctx->sample_rate,
                ffmpeg_ctx->frame->nb_samples,
                (const uint8_t **)ffmpeg_ctx->frame->extended_data,
                ffmpeg_ctx->frame->nb_samples);

    gettimeofday(&start_time, NULL);
    ffmpeg_ctx->audio_dst_buffer_size_real = \
                    av_samples_get_buffer_size(
                            NULL,
                            ffmpeg_ctx->audio_dec_ctx->channels,
                            ffmpeg_ctx->frame->nb_samples,
                            ffmpeg_ctx->audio_dec_ctx->sample_fmt,
                            1
                    );
    // LV_LOG_WARN("audio_dst_buffer_size_real : %d", ffmpeg_ctx->audio_dst_buffer_size_real);

    // LV_LOG_WARN("frame->nb_samples : %d", ffmpeg_ctx->frame->nb_samples);

#if EANBLE_THREAD_PLAY
    pthread_mutex_lock(&ffmpeg_ctx->audio_dst_mutex);
    pthread_cond_signal(&ffmpeg_ctx->audio_dst_cond);
    pthread_mutex_unlock(&ffmpeg_ctx->audio_dst_mutex);
#else
#if USE_ALSA_LIB
    int frame_size = ffmpeg_ctx->audio_dst_buffer_size_real/(2*16/8);
    ret = snd_pcm_writei(ffmpeg_ctx->pcm_handle,
                        ffmpeg_ctx->audio_dst_buffer,
                        frame_size);

    // if(-EPIPE == ret)
    // {
    //     snd_pcm_prepare(ffmpeg_ctx->pcm_handle);
    // }
    ...
    // }
#else
    int written_frames = pcm_writei(ffmpeg_ctx->pcm_handle,
                                    ffmpeg_ctx->audio_dst_buffer,
                                    ffmpeg_ctx->audio_dst_buffer_size_real);
    // int written_frames = pcm_writei(ffmpeg_ctx->pcm_handle,
    //                                 ffmpeg_ctx->audio_dst_buffer,
    //                                 pcm_bytes_to_frames(ffmpeg_ctx->pcm_handle, 
    //                                                     ffmpeg_ctx->audio_dst_buffer_size_real)
    //                                 );
    if (written_frames < 0) {
        LV_LOG_WARN("error playing ! %s", pcm_get_error(ffmpeg_ctx->pcm_handle));
    }
#endif
#endif

    gettimeofday(&end_time, NULL);
    // LV_LOG_WARN("time cost : %ld us", end_time.tv_usec - start_time.tv_usec);
...
static int ffmpeg_get_frame_refr_period_audio(struct ffmpeg_context_s * ffmpeg_ctx)
{
    int avg_frame_rate_num = ffmpeg_ctx->audio_stream->avg_frame_rate.num;
    if(avg_frame_rate_num > 0) {
        int period = 1000 * (int64_t)ffmpeg_ctx->audio_stream->avg_frame_rate.den
                     / avg_frame_rate_num;
        return period;
    }
    ...
    while(1) {
        ...
        if(av_read_frame(ffmpeg_ctx->fmt_ctx, &(ffmpeg_ctx->pkt)) >= 0) {
            ...
            if(ffmpeg_ctx->pkt.stream_index == ffmpeg_ctx->video_stream_idx) {
                ...
                is_image = true;
            }else if(ffmpeg_ctx->pkt.stream_index == ffmpeg_ctx->audio_stream_idx) {
                ret = ffmpeg_decode_packet(ffmpeg_ctx->audio_dec_ctx,
                                           &(ffmpeg_ctx->pkt), ffmpeg_ctx);
                // ret = avcodec_decode_audio4(ffmpeg_ctx->audio_dec_ctx,
                //                             ffmpeg_ctx->frame,
                //                             &got_frame,
                //                             ffmpeg_ctx->pkt);
                // if (0 < got_frame) {
...
struct ffmpeg_context_s * ffmpeg_open_file(const char * path)
{
    ...
    for (unsigned int s = 0; s < ffmpeg_ctx->fmt_ctx->nb_streams; ++s)
	{
        LV_LOG_WARN("s : %d", s);
        av_dump_format(ffmpeg_ctx->fmt_ctx, s, path, 0);
        if(ffmpeg_ctx->fmt_ctx->streams[s]->codecpar->codec_type == AVMEDIA_TYPE_AUDIO){
            LV_LOG_WARN("AVMEDIA_TYPE_AUDIO");
            ffmpeg_ctx->audio_stream_idx = s;
        }
        else if(ffmpeg_ctx->fmt_ctx->streams[s]->codecpar->codec_type == AVMEDIA_TYPE_VIDEO) {
            LV_LOG_WARN("AVMEDIA_TYPE_VIDEO");
            ffmpeg_ctx->video_stream_idx = s;

        }
    }

    // av_dump_format(ffmpeg_ctx->fmt_ctx, 0, 0, 0);
    // ffmpeg_ctx->video_stream_idx = av_find_best_stream(ffmpeg_ctx->fmt_ctx,
    //                                                 AVMEDIA_TYPE_VIDEO,
    //                                                 ffmpeg_ctx->video_stream_idx,
    //                                                 -1,
    //                                                 NULL, 0);
    // ffmpeg_ctx->audio_stream_idx = av_find_best_stream(ffmpeg_ctx->fmt_ctx,
    //                                                     AVMEDIA_TYPE_AUDIO,
    //                                                     ffmpeg_ctx->audio_stream_idx,
    //                                                     ffmpeg_ctx->video_stream_idx,
    //                                                     NULL, 0);
    LV_LOG_WARN("video_index:%d, audio index:%d",ffmpeg_ctx->video_stream_idx, ffmpeg_ctx->audio_stream_idx);                                                        

    /* config audio codec here */
    if (ffmpeg_ctx->audio_stream_idx >= 0)
    {
        int ret = 0;
        int period;

        ffmpeg_ctx->audio_dec = avcodec_find_decoder(ffmpeg_ctx->fmt_ctx->streams[ffmpeg_ctx->audio_stream_idx]->codecpar->codec_id);
        ffmpeg_ctx->audio_dec_ctx = avcodec_alloc_context3(ffmpeg_ctx->audio_dec);

        avcodec_parameters_to_context(ffmpeg_ctx->audio_dec_ctx, ffmpeg_ctx->fmt_ctx->streams[ffmpeg_ctx->audio_stream_idx]->codecpar);
	    avcodec_open2(ffmpeg_ctx->audio_dec_ctx, ffmpeg_ctx->audio_dec, NULL);
        LV_LOG_WARN("decoder name : %s", ffmpeg_ctx->audio_dec->name);

        ffmpeg_ctx->audio_src_format = ffmpeg_ctx->audio_dec_ctx->sample_fmt;
        ffmpeg_ctx->audio_dst_format = AV_SAMPLE_FMT_S16;

        ffmpeg_ctx->audio_src_sample_rate = ffmpeg_ctx->audio_dec_ctx->sample_rate;
        ffmpeg_ctx->audio_dst_sample_rate = ffmpeg_ctx->audio_dec_ctx->sample_rate;

        ffmpeg_ctx->audio_src_ch_layout = ffmpeg_ctx->audio_dec_ctx->channel_layout;
        ffmpeg_ctx->audio_dst_ch_layout = av_get_channel_layout_nb_channels(AV_CH_LAYOUT_STEREO);

        ffmpeg_ctx->audio_src_nb_channels = av_get_channel_layout_nb_channels(ffmpeg_ctx->audio_src_ch_layout);
        ffmpeg_ctx->audio_dst_nb_channels = av_get_channel_layout_nb_channels(ffmpeg_ctx->audio_dst_ch_layout);

        // ret = av_samples_alloc_array_and_samples(&ffmpeg_ctx->audio_src_data,
        //                                         &ffmpeg_ctx->audio_src_linesize,
        //                                         ffmpeg_ctx->audio_src_nb_channels,
        //                                         ffmpeg_ctx->audio_src_nb_samples,
        //                                         ffmpeg_ctx->audio_src_format,0);
        // if(ret < 0) {
        //     LV_LOG_ERROR("Could not alloc source samples");
        // }

        // ffmpeg_ctx->audio_max_dst_nb_samples = ffmpeg_ctx->audio_dst_nb_samples 
        //     = av_rescale_rnd(ffmpeg_ctx->audio_src_nb_samples,
        //                     ffmpeg_ctx->audio_dst_sample_rate,
        //                     ffmpeg_ctx->audio_src_sample_rate,
        //                     AV_ROUND_UP);

        
        // ret = av_samples_alloc_array_and_samples(&ffmpeg_ctx->audio_dst_data,
        //                                         &ffmpeg_ctx->audio_dst_linesize,
        //                                         ffmpeg_ctx->audio_dst_nb_channels,
        //                                         ffmpeg_ctx->audio_dst_nb_samples,
        //                                         ffmpeg_ctx->audio_dst_format,0);                                                                                          
        // if(ret < 0) {
        //     LV_LOG_ERROR("Could not alloc destination samples");
        // }
        /* alloc the output buffer */
        ffmpeg_ctx->audio_dst_buffer = (uint8_t *)av_malloc(MAX_AUDIO_FRAME_SIZE * 4);

        ffmpeg_ctx->audio_swr_ctx = swr_alloc();
        ffmpeg_ctx->audio_swr_ctx = swr_alloc_set_opts(
                        NULL,
                        AV_CH_LAYOUT_STEREO,
                        AV_SAMPLE_FMT_S16,
                        ffmpeg_ctx->audio_dec_ctx->sample_rate,
        ...
        if(ret = swr_init(ffmpeg_ctx->audio_swr_ctx) < 0) {
            LV_LOG_ERROR("Failed to initialize the resampling context");
        }

        LV_LOG_WARN("Audio Input Infomation :");
        LV_LOG_WARN("\t Format : %d", ffmpeg_ctx->audio_src_format);
        LV_LOG_WARN("\t Sample Rate : %d", ffmpeg_ctx->audio_src_sample_rate);
        LV_LOG_WARN("\t Channle Layout : %d", ffmpeg_ctx->audio_src_ch_layout);
        
        LV_LOG_WARN("Audio Output Infomation :");
        LV_LOG_WARN("\t Format : %d", ffmpeg_ctx->audio_dst_format);
        LV_LOG_WARN("\t Sample Rate : %d", ffmpeg_ctx->audio_dst_sample_rate);
        LV_LOG_WARN("\t Channle Layout : %d", ffmpeg_ctx->audio_dst_ch_layout);

        // int avg_frame_rate_num = ffmpeg_ctx->audio_stream->avg_frame_rate.num;
        // int period = 1000 * (int64_t)ffmpeg_ctx->audio_stream->avg_frame_rate.den
        //              / avg_frame_rate_num;
        // LV_LOG_WARN("period: %d", period);
        snd_pcm_open(&ffmpeg_ctx->pcm_handle, "default", SND_PCM_STREAM_PLAYBACK, 0);
        
#if EANBLE_THREAD_PLAY
        /* start a pcm play thread */
        pthread_t tid;
        pthread_create(&tid, NULL, snd_pcm_writei_thread, ffmpeg_ctx);
#endif
    }
...
#if USE_ALSA_LIB
    snd_pcm_close(ffmpeg_ctx->pcm_handle);
#else
    pcm_close(ffmpeg_ctx->pcm_handle);
#endif
    free(ffmpeg_ctx);
